...
export default function IterateFeedback({
  ...
  const handleImplement = async (item: Feedback) => {
    setImplementState("implementing");
    setImplementSteps([]);
    setImplementFigmaUrl(null);
    ...
    try {
      const res = await fetch("/api/implement", {
        ...
        body: JSON.stringify({
          ...
          imageMediaType,
          projectTitle,
          feedbackItems: [{ level: item.level, text: item.text, rationale: item.rationale }],
        }),
      });
          ...
          <div className="p-3 flex flex-col gap-2" style={{ borderBottom: "1px solid #D4DEE9" }}>
            ...
            {iteratingItem.rationale && (
              <div className="flex flex-col gap-2 p-2 rounded-[4px]" style={{ background: "#F4F7FA" }}>
                ...
                {implementState === "implementing" && (
                  <div className="flex flex-col gap-1.5">
                    ...
                    {implementSteps.length > 0 && (
                      <p className="text-[10px] text-gray-400 truncate">{implementSteps[implementSteps.length - 1]}</p>
                    )}
                  </div>
                )}
                ...
                {/* Error state */}
                {implementState === "error" && (
                  <p className="text-[11px] text-red-500">Something went wrong. Try again.</p>
                )}

                {/* Implement button — only shown when idle */}
                {implementState === "idle" && (
                  <div className="flex justify-end">
                    <button
                      onClick={() => handleImplement(iteratingItem)}
